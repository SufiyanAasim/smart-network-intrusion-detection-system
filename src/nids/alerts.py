"""Alerting for critical-threat detections (Slack / email / generic webhook).

Every channel is optional and configured entirely via environment variables,
so the app runs with zero alerting configured (calls just no-op). Kept free
of Streamlit imports so it's independently testable/mockable.
"""
...
def _post_json(url, message, timeout=5):
